...
#### The relationship between diabetes rate and the average supermarkets & grocery stores/1,000 residents.
```{r average_stores}
health_store = store %>% 
  group_by(state) %>% 
  ...
  filter(!(duplicated(state))) %>% 
  left_join(diab2009, by = "state") %>% 
  rename(diabetes_2009 = percentage) %>% 
  left_join(diab2014, by = "state") %>% 
  rename(diabetes_2014 = percentage) %>% 
  left_join(medhincome_13, by = "state")

p1 = health_store %>% 
ggplot(aes(x = store_rate_2009, y = diabetes_2009)) +
  geom_point(aes(color = `income level`),alpha = .7) +
  geom_smooth(method = "lm",formula = y~x) +
  ...
  labs(title = "",
    x = "Average Store/1000 residents",
    y = "Diabetes Rate(%)") +
  theme(legend.position = "bottom") 

p2 = health_store %>% 
ggplot(aes(x = store_rate_2014, y = diabetes_2014)) +
  geom_point(aes(color = `income level`),alpha = .7) +
  geom_smooth(method = "lm",formula = y~x) +
  ggpmisc::stat_poly_eq(formula = y~x,
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
...
cowplot::plot_grid(p1, p2, labels = c("2009","2014"))
```
